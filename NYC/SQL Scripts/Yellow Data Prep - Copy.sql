...
DECLARE 
@CSVFilePath				VARCHAR(100)='D:\Big Data\CSV\' + @DataKey + '\yellow_tripdata~.csv',
@FileGroup					VARCHAR(100)='FG_' + @DataKey + '~',
@NDFIlePath					VARCHAR(100) ='C:\TaxiData\'+ @dataKey+'\~.ndf',
@NDFFileName				VARCHAR(100)='P_' + @DataKey + '~'

DECLARE @FormatBefore_2015		VARCHAR(1000)='D:\Big Data\FormatFile\Yellow_FormatBefore_2015.xml'
DECLARE @Format2015_2016_B4_07	VARCHAR(1000)='D:\Big Data\FormatFile\Yellow_Format2015_2016_B4_07.xml'
DECLARE @FormatAfter_2016_07	VARCHAR(1000)='D:\Big Data\FormatFile\Yellow_FormatAfter_2016_07.xml'
 ...
 *******************************************/
 ...
 BEGIN
	...
	*/
	...
	/*
	SET @ColumnList = CASE 
	                       WHEN LEFT(@tmpFileVal, 4) < 2015 THEN @YellowColumnsBefore_2015
	                       WHEN LEFT(@tmpFileVal, 4) = 2015 OR (
	                                LEFT(@tmpFileVal, 4) = 2016
	                                AND CAST(RIGHT(@tmpFileVal, 2) AS INT) < 7
	                            ) THEN @YellowColumns_2015_2016_B4_07
	                       ELSE @YellowColumnsAfter_2016_07
	                  END 
		*/
	...
	--SET @SQLForAsyncRun=REPLACE( REPLACE(@SQLForAsyncRun,'!!FileName',@DataKey + @tmpFileVal),'!!!ActualSQL',@SQL)
	
	--PRINT @SQLForAsyncRun
	
	--DELETE FROM TripInsertStmnt WHERE [FileName] = @DataKey + @tmpFileVal
	--INSERT INTO TripInsertStmnt	([FileName],[Query]	) VALUES (@DataKey + @tmpFileVal , @SQLForAsyncRun)
